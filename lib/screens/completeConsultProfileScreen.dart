...
class CompleteConsultProfileScreen extends StatefulWidget {
  ...
  const CompleteConsultProfileScreen({Key key, this.user}) : super(key: key);
  @override
...
class _CompleteConsultProfileScreenState extends State<CompleteConsultProfileScreen> {
  ...
  void initState() {
    ...
    widget.user.languages=["العربية","English"];
  ...
  void didChangeDependencies() {
    ...
    _serviceType = [getTranslated(context, "jeras1"),getTranslated(context, "perfect"), getTranslated(context, "glorified"),getTranslated(context, "vocal")];
    ...
    if(widget.user.consultType==null)
      setState(() {
        _serviceTypeValue=getTranslated(context, "jeras1");
      });
    else if(widget.user.consultType=="jeras")
      setState(() {
        _serviceTypeValue=getTranslated(context, "jeras1");
      });
    else
      setState(() {
        _serviceTypeValue=getTranslated(context, widget.user.consultType);
      });
                ...
                child: ListView(physics: NeverScrollableScrollPhysics(),controller: scrollController,
                  children:  [
                    ...
                    Row(mainAxisAlignment: MainAxisAlignment.center,
                      children: [
                        Text(
                          getTranslated(context, "serviceType"),
                          style: GoogleFonts.cairo(
                            color: Colors.white,
                            fontSize: 15.0,
                            fontWeight: FontWeight.bold,
                    ...
                    StatefulBuilder(builder: (context, setState) {
                      return RadioGroup<String>.builder(
                        ...
                        groupValue:_serviceTypeValue,//getTranslated(context, _serviceTypeValue) ,
                        horizontalAlignment: MainAxisAlignment.spaceAround,
                        onChanged: (value) =>
                            setState(() {
                              _serviceTypeValue = value;
                            }),
  ...
  save() async {
    ...
    if(userName==null||bio==null||price==null||userName==""||bio==""||price==""||widget.user.workDays.length==0||
        ...
        _serviceTypeValue==""||_serviceTypeValue==null){
    ...
    else
    {
      ...
      String consultType="perfect";
      if(_serviceTypeValue==getTranslated(context, "glorified"))
       setState(() {
         consultType="glorified";
       });
      else if(_serviceTypeValue==getTranslated(context, "jeras")||_serviceTypeValue==getTranslated(context, "jeras1"))
       setState(() {
         consultType="jeras";
       });
      else if(_serviceTypeValue==getTranslated(context, "vocal"))
        setState(() {
          consultType="vocal";
        });
      else
      ...
      widget.user.consultType=consultType;
      //============packages
      ...
      _workTime.from=from;
      _workTime.to=to;
      widget.user.languages=["English","العربية"];
      widget.user.chat=true;
      widget.user.voice=true;
